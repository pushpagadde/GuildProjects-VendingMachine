/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.sg.section03unittests;

import org.junit.After;
import org.junit.AfterClass;
import org.junit.Before;
import org.junit.BeforeClass;
import org.junit.Test;
import static org.junit.Assert.*;

/**
 *
 * @author apprentice
 */
public class PlayOutsideTest {
    PlayOutside play = new PlayOutside();
    
    public PlayOutsideTest() {
    }
    
    @BeforeClass
    public static void setUpClass() {
    }
    
    @AfterClass
    public static void tearDownClass() {
    }
    
    @Before
    public void setUp() {
    }
    
    @After
    public void tearDown() {
    }

    @Test
    public void testIsSummerLowTemp() {
        assertFalse(play.playOutside(50, true));
    }
    @Test
    public void testIsSummerHighTemp() {
        assertFalse(play.playOutside(101, true));
    }
    @Test
    public void testNotSummerHighTemp() {
        assertFalse(play.playOutside(101, false));
    }
    @Test
    public void testNotSummerLowTemp() {
        assertFalse(play.playOutside(59, false));
    }
    @Test
    public void testIsSummerOkTemp() {
        assertTrue(play.playOutside(61, true));
    }
    
}
